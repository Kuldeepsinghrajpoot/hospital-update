...
async function getPatients({ id }: { id: string }) {
    const response = await fetch(`${process.env.URI}/api/admin/patients-chart?id=${id}`)
...
export default async function Dashboard({ role }: { role: any }) {
    ...
    const patients = await getPatients(Session?.user._id)
